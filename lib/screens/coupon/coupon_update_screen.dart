...
class CouponUpdateScreen extends StatefulWidget {
  final Coupon coupon;
  final Function onUpdate;

  const CouponUpdateScreen({
    super.key,
  ...
  @override
  State<CouponUpdateScreen> createState() => _CouponUpdateScreenState();
}
...
class _CouponUpdateScreenState extends State<CouponUpdateScreen> {
  ...
  late List<CouponType?> initialValueType;
  late List<CouponAccess?> initialValueAccess;
  ...
  Future<void> fetchData() async {
    ...
    if (context.mounted) {
      ...
      initialValueType = couponType
          .where(
            (coupon) => widget.coupon.type.any((type) => coupon.id == type.id),
          )
          .toList();

      initialValueAccess = couponAccess
          .where(
            (coupon) =>
                widget.coupon.access.any((access) => coupon.id == access.id),
          )
          .toList();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(context.loc.update_coupon_title),
      ),
      body: FutureBuilder(
  ...
  Widget percentageInput(BuildContext context) {
    return Visibility(
      visible: selectedCouponAccess.isEmpty
          ? widget.coupon.access
      ...
      child: Column(
        children: [
          ...
          TextFormField(
            initialValue: widget.coupon.percentage.toString(),
    ...
    return Visibility(
      ...
      child: Column(
        children: [
          ...
          TextFormField(
            initialValue: widget.coupon.value.toString(),
            keyboardType: TextInputType.number,
            decoration: InputDecoration(
    ...
    return Column(
      children: [
        ...
        MultiSelectBottomSheetField<CouponAccess?>(
          initialValue: initialValueAccess,
          buttonText: Text(
              ...
              if (context.mounted) {
                await loadingMessageTime(
                  ...
                  subtitle: variableFromService == EnumCoupon.success
                      ? context.loc.update_coupon_success_subtitle
